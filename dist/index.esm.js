import { makeVar, InMemoryCache, defaultDataIdFromObject, ApolloClient, from, useQuery, useMutation, NetworkStatus, useLazyQuery, ApolloProvider, useReactiveVar } from '@apollo/client';
import React, { useMemo, useRef, useEffect, useState, createContext, useContext, useCallback } from 'react';
...
const customizationSchemaVar = makeVar({});
...
const useApollo = (appProps, apolloConfig, _storeId) => {
    ...
    return useMemo(() => initializeApollo(storeId, state, apolloConfig), [storeId, state]);
};
...
const useProductsFilterParams = (skip) => {
    ...
    const [_search, setSearch] = useState(searchParams.search ?? '');
    const [selectedOptionValues, setSelectedOptionValues] = useState(searchParams.custom_option_values ?? []);
    const [selectedCategory, setSelectedCategory] = useState(searchParams.category ?? '');
    const [selectedHasDiscount, setSelectedHasDiscount] = useState(!!searchParams.has_discount);
    const [selectedHasStock, setSelectedHasStock] = useState(!!searchParams.has_stock);
...
const useProducts = (customQuery = {}) => {
    ...
    const products = useMemo(() => data?.customer?.getProducts?.results.map((product) => useMiniProduct(product)) || [], [data]);
...
const useProduct = () => {
    ...
    const { basket } = useStore();
    const { data: d, loading, error, } = useQuery(GET_PRODUCT, {
        variables: {
    ...
    const { label, voter_number, average_score, profit_percent, productId, category, tax, has_loyalty_gift, feedback_count, single_tax, } = useMemo(() => {
        return {
            productId: data?.id,
    ...
    const stockUpdatedSelectedVariant = useMemo(() => {
        if (!selectedVariant)
            return null;
...
const useSuggestionProducts = () => {
    ...
    const products = useMemo(() => data?.customer.getSuggestionProducts?.results.map((product) => useMiniProduct(product)) || [], [data]);
    return {
        data: products,
        loading,
        error: apolloError(error),
...
const useUser = () => {
    const { error: fetchError, loading: fetchLoading, data, refetch, networkStatus, } = useQuery(GET_PROFILE, {
    ...
    const [loyaltyCreditStateData, setLoyaltyCreditStateData] = useState(0);
    ...
    const loyaltyCredit = useMemo(() => loyaltyCreditStateData, [loyaltyCreditStateData]);
    const handleCancel = () => {
        if (data) {
            const info = { ...data.customer.getProfile };
            delete info['__typename'];
...
const useForgetPassword = () => {
    ...
    const [step, setStep] = useState('verifyPhoneNumber');
    const [error, setError] = useState({});
...
const useAuth = (autoRedirectOnLogin = true) => {
    ...
    const [step, setStep] = useState('fristPage');
...
const useStoreInfo = () => {
    ...
    const [createStoreOpeningNotifier, { loading: storeOpeningNotifierLoading }] = useMutation(CREATE_STORE_OPENING_STORE_NOTIFIER);
    const [storeOpeningNotifierError, setStoreOpeningNotifierError] = useState({});
...
const withApollo = (C) => (...queries) => {
    const Component = (props) => jsx(C, { ...props });
...
const useBasket$1 = () => {
    ...
    const [updateBasketLoading, setUpdateBasketLoading] = useState(false);
    const [items, setItems] = useState(localStorageBasket.items);
    const [address, setAddress] = useState(localStorageBasket.address);
    const [receiverInfo, setReceiverInfo] = useState(localStorageBasket.receiverInfo);
    const [shipping, setShipping] = useState(localStorageBasket.shipping);
    const [transaction, setTransaction] = useState(localStorageBasket.transaction);
    const [packing, setPacking] = useState(localStorageBasket.packing);
    const [discount, setDiscount] = useState(localStorageBasket.discount);
    const [useLoyalty, setUseLoyalty] = useState(localStorageBasket.useLoyalty);
    const [description, setDescription] = useState(localStorageBasket.description);
    useEffect(() => {
    ...
    const data = {
        items: useMemo(() => (items ? items : []), [items]),
        address: useMemo(() => address, [address]),
        packing: useMemo(() => packing, [packing]),
        discount: useMemo(() => discount, [discount]),
        useLoyalty: useMemo(() => useLoyalty, [useLoyalty]),
        shipping: useMemo(() => shipping, [shipping]),
        receiverInfo: useMemo(() => receiverInfo, [receiverInfo]),
        transaction: useMemo(() => transaction, [transaction]),
        description: useMemo(() => description, [description]),
    };
    return {
        data,
        initialSync,
...
const StoreProvider = (props) => {
    ...
    return jsx(StoreContext.Provider, { value: value, children: props.children });
};

const ThemeProvider = (props) => {
    return (jsx(ApolloProvider, { client: props.apolloClient, children: jsx(StoreProvider, { customization: props.customization, children: props.children }) }));
};
...
const initializeApp = (Component, themeConfig, queries = [
    ...
    const MyApp = (props) => {
        ...
        return (jsx(ThemeProvider, { customization: themeConfig.customization, apolloClient: client, children: jsx(Component, { ...props }) }));
    };
...
const useOrderActions = (onCompletedAction) => {
    ...
    const [getGatewayLoading, setGetGatewayLoading] = useState(false);
    const [error, setError] = useState({});
    const handleRePayOrder = useCallback(async (orderId) => {
    ...
    const handleSubmitCardToCardPayment = useCallback(async (orderId, image) => {
        setError({});
        return new Promise((resolve, reject) => {
            paymentCard({
    ...
    const handleUnreceivedOrder = useCallback(async (orderId, receivedStatus) => {
        setError({});
        return new Promise((resolve, reject) => {
            unrecivedOrder({
                variables: {
...
const useOrdersList = () => {
    const limit = 10;
    const searchParams = useOrdersSearchParams();
    const { data: d, error, fetchMore, loading, } = useQuery(GET_ORDERS, {
        notifyOnNetworkStatusChange: true,
        fetchPolicy: 'network-only',
        variables: {
            params: searchParams,
        ...
        if (data) {
            ...
            for (const order of results) {
                ...
                _orders.push({
                    ...
                    createdAt: moment(createdAt).format('jYYYY/jMM/jDD'),
                    cost,
...
const useAddress = () => {
    const { data: queryData, loading: fetchLoading, error: fetchError, } = useQuery(GET_ADDRESSES, {
        skip: !isUserLoggedIn(),
        context: {
            headers: {
                'accept-language': 'fa-IR',
...
const useFavorites = () => {
    ...
    const { data: dataFavorites, error: fetchError, loading: fetchLoading, fetchMore, refetch, } = useQuery(GET_FAVORITES, {
...
const useShippingMethod = () => {
    ...
    const [stateData, setStateData] = useState([]);
    const [approximateSendingDate, setApproximateSendingDate] = useState('');
    useEffect(() => {
        if (sokectShpingList)
...
const useTransactionType = ( /*{ totalCost }: { totalCost: number }*/) => {
    const { pathname, push, query } = useRouter();
    const { data: queryData, loading, error, } = useQuery(GET_TRANSACTION_TYPES, {
        skip: !isUserLoggedIn(),
        context: {
            headers: {
                'accept-language': 'fa-IR',
...
const usePacking = () => {
    ...
    const { data: queryData, error, loading, } = useQuery(GET_POCKET, {
...
const useLoyaltyCredit = () => {
    const { error, loading, data: queryData, } = useQuery(GET_LOYALTY_CREDIT, {
        skip: !isUserLoggedIn(),
        context: {
            headers: {
                'accept-language': 'fa-IR',
...
const useLoyaltyGift = () => {
    const { earning, is_earning_loyalty_active } = useQuery(GET_STORE_INFO, { context: { headers: { 'accept-language': 'fa-IR' } } }).data?.customer
        .getStoreInfo || {};
    return useMemo(() => {
        return {
...
const useCustomization = (modelName) => {
    ...
    const [tempState, setTempState] = useState(initialData);
    useEffect(() => {
        if (!didMountRef.current) {
            didMountRef.current = true;
        }
...
export { ADD_FAVORITES, CANCEL_ORDER, CHANGE_PASSWORD_WITHOUT_OTP, CLEAR_COOKIE, CREATE_ADDRESS, CREATE_ORDER, CREATE_PROFILE, CREATE_STORE_OPENING_STORE_NOTIFIER, CREATE_THEME_CUSTOMIZATION, DELETE_ADDRESS, DynamicComponentsContainer, FORGET_PASSWORD, GET_ADDRESSES, GET_APPEARANCE, GET_APPROXIMATE_SENDING_DATE, GET_BASKET, GET_BLOG_ARTICLE, GET_BLOG_ARTICLES, GET_BLOG_HIGHLIGHTS, GET_BREADCRUMB, GET_CATEGORIES, GET_CHANGE_AUTH_PASSWORD, GET_COOKIE, GET_FAVORITES, GET_GATEWAY, GET_LOYALTY_CREDIT, GET_LOYALTY_LOGS, GET_NOTIFICATION_SETTING, GET_ORDER, GET_ORDERS, GET_ORDERS_STATUS_COUNT, GET_POCKET, GET_PRODUCT, GET_PRODUCTS, GET_PRODUCTS_FEEDBACK, GET_PRODUCT_FILTER_PARAMS, GET_PROFILE, GET_REFRESH_TOKEN, GET_RETURNED_ORDER, GET_SHIPPING_ADDRESS, GET_SHIPPING_ADDRESS_SOCKET, GET_SITE_MAP, GET_STORE_INFO, GET_SUGGESTION_PRODUCTS, GET_THEME_CUSTOMIZATION, GET_TOKEN, GET_TRANSACTION_TYPES, GET_USER_TYPE, GET_VARIANTS_STOCK, GET_WEBSOCKET_NOTIF, IS_FAVORITE, LIKE_BLOG_ARTICLE, OTP_SEND, OTP_SEND_SIGNUP, OTP_SEND_V2, PAYMENT_CARD, REMOVE_FAVORITES, RETURN_ORDER, UPDATE_ADDRESS, UPDATE_BASKET, UPDATE_NOTIFICATION_SETTINGS, UPDATE_PROFILE, UPDATE_RECIEVE_STATUS, UPDATE_THEME_CUSTOMIZATION, UPLOAD_IMAGE, VOUCHER_CHECK, apolloLocalState, clearNextCookie, handleRefreshToken, initializeApp, isUserLoggedIn, setNextCookie, siteMap, ssrQueries, staticLinks, updateProfileMutatuin, useAddress, useAuth, useBasket, useBlogArticle, useBlogArticles, useBlogHighlights, useBreadcrumb, useCategories, useCustomization, useFavorites, useImageUpload, useLoyaltyLogs, useNotificationSettings, useOrderDetail, useOrders, useProduct, useProductFeedback, useProducts, useRefundOrder, useReturnedOrderDetail, useReturnedOrderInvoice, useReturnedOrderItems, useReturnedOrderStatus, useSelectableOrder as useSelectableItems, useSokect, useStore, useStoreInfo, useSuggestionProducts, useTheme, useUser, withApollo };
